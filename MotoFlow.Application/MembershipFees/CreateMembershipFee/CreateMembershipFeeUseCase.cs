...
public class CreateMembershipFeeUseCase : ICreateMembershipFeeUseCase
{
    ...
    public CreateMembershipFeeUseCase(
    ...
    public async Task ExecuteAsync(
        string memberId,
        CreateMembershipFeeRequest createMembershipFeeRequest,
        CancellationToken cancellationToken)
    {
        var memberGuid = Guid.Parse(memberId);

        var feeAlreadyExists =
            await _membershipFeeRepository.ExistsForMemberAndPeriodAsync(
                memberGuid,
        ...
        if (feeAlreadyExists)
        {
            throw new ConflictException(
                "Já existe uma mensalidade para este membro neste período.");
        }
        ...
        await _membershipFeeRepository.AddAsync(
            fee,
            cancellationToken);
